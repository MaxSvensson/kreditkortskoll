...
const CompareBox = () => {
    ...
    const removeFromCompare = () => {
        ...
        const index = State.compareIds.findIndex((Card : CreditCardInterface) => Card.id)
        if(index < 0) return;
    ...
    return <div className={styles.CompareBox}>
        ...
        <div className={styles.buttonBackground}>
            <Link href={`/jamfor?cards=${JSON.stringify(state.compareIds.map((Card : CreditCardInterface) => Card.id))}`} >
